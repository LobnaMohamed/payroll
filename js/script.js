...
$(document).ready(function() {
    ...
    $(document).on('click', '.editEmpData', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#syndicatecurrentIDValue').val(data.syndicate_id);


                $('#syndicatecurrentValueEdit').val(data.currentSyndicate);
                $('#syndicatecurrentValue').val(data.currentSyndicate);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (currentURL == 'empdata.php') {
                    //console.log(data);
                    $('#empDatabody').html(data);
                } else if (currentURL == 'empdatatable.php') {
                    $('#empDataTablebody').html(data);
                } else if (currentURL == 'timesheet.php') {
                    // console.log(data);
                    $('#timesheetbody').html(data);
                ...
                } else if (currentURL == 'appraisals.php') {
    ...
    $(document).on('click', '.edittimsesheetData', function() {
        ...
        $.ajax({
            ...
            method: "POST",
            data: {
                editTimesheet_empID: employee_id,
    ...
    $('.delete_employee').on('click', function() {
        if (confirm("سيتم حذف الموظف نهائياً .. هل أنت متأكد؟")) {
            ...
            $.ajax({
                url: "fetch.php",
                method: "POST",
                data: { empIDForDelete: empID },
                success: function(data) {
                    //console.log(data);
                    tablerow.css("text-decoration", "line-through");
                    tablerow.fadeOut(1200, function() {
                        tablerow.remove();
                    });

                },
                error: function(error) {
                    alert(" لم يتم حذف الموظف بنجاح..حاول مرة أخرى ");
                    //console.log(error);
                }
            });
