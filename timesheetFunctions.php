	...
	function getinsertTimesheet(){
		...
		if(!empty($_POST['dateFrom'])){	
			$sql = "select  e.empName,e.currentCode,e.ID
					from	employee e
					where e.is_deleted=0 and e.ID not in (select empt.emp_id
										from timesheets t inner join empTimesheet empt on t.ID = empt.TS_id 
										where month(t.sheetDate)= month( '".$_POST['dateFrom']."')
										and year(t.sheetDate)= year('".$_POST['dateFrom']."'))";
		}
		...
		if(isset($_POST["insertovernight"])){
			foreach ($_POST['overnight_days'] as $empID => $value) {
				...
				if($overnight >0 ){
					...
					$updateinEmptimsheetsql = "update emptimesheet set overnight_days = $overnight 
												where emp_id =$empID and TS_id = $lastID ";
					//echo $sql;
					$stmt = $con->prepare($updateinEmptimsheetsql);
					$stmt->execute();
					...
					if(in_array($file_extension, $allowed_extension)){
						...
						for($row =1; $row < $count ; $row ++){
							...
							if($empID){
								...
								$updateovernightinEmptimesheet_sql = "update emptimesheet 
																set overnight_days = ".$data[$row][2]." 
																where emp_id = $empID and TS_id = $lastID; ";
								// echo $insertovernight_sql . "<br>";
								$updatesql .=$updateovernightinEmptimesheet_sql;
						...
						$statement->execute();		
						$statement = $con->prepare($updatesql);
		...
		if(isset($_POST["insertshift"])){
			...
			foreach ($_POST['shift_days'] as $empID => $value) {
				...
				if($shift >0 ){
					...
					$updateinEmptimsheetsql = "update emptimesheet set shift_days = $shift 
												where emp_id =$empID and TS_id = $lastID ";
					...
					if(in_array($file_extension, $allowed_extension)){
						...
						for($row =1; $row < $count ; $row ++){
							...
							if($empID){
								...
								$updateshiftinEmptimesheet_sql = "update emptimesheet 
																set shift_days = ".$data[$row][2]." 
																where emp_id = $empID and TS_id = $lastID; ";
								// echo $insertovernight_sql . "<br>";
								$updatesql .=$updateshiftinEmptimesheet_sql;
					...
					if(in_array($file_extension, $allowed_extension)){
						...
						for($row =1; $row < $count ; $row ++){
							...
							if($empID){
								...
								$updatesickleavesinEmptimesheet_sql = "update emptimesheet 
																set real_sickLeaves = ".$data[$row][2]." 
																where emp_id = $empID and TS_id = $lastID; ";
								// echo $insertovernight_sql . "<br>";
								$updatesql .=$updatesickleavesinEmptimesheet_sql;
